...
Under releases, you may run the shell install script `install.sh` which will:

- Clone the repository
- Install required `Python` dependencies

You can skip to `How to use the agent & demo run` after running the install script.

Alternatively, you may follow the steps below for a manual install.
...
For the Model Context Protocol, FastMCP is used.
...
The MCP server should populate in your terminal:

```shell
PS python server.pyocuments\GitRepos\Software_Testing_Agent>
...
[11/17/25 16:27:41] INFO     Starting MCP server 'software-tester' with transport 'sse' on http://127.0.0.1:8000/sse                                                                                                        server.py:2055
INFO:     Started server process [44660]
...
**Copy the URL** and add the MCP server:

`CTRL+SHIFT+P` -> `MCP: Add Server` -> `HTTP` -> Paste the URL and hit `Enter`
...
A good first prompt to ensure the agent is communicating to the MCP and using the utilities properly is:
...
**_How does the agent integrate with Github workflows?_**
...
**_What if the codebase the agent is working on has a bug, or the agent creates a bug in a created, how is it handled?_**
...
**_My agent cannot find the Jacoco report / complete operations, what do I do?_**
...
**_My current tests have build failures, can the agent fix these errors?_**
...
**_Does this agent support Junit 4?_**
